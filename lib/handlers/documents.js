...
function createSobjectToDocument(org, oauth, search) {
  return function sobjectToDocument(sobject, cb) {
    async.waterfall([
        ...
        function recordToDocument(record, cb) {
          cb(null, docFormatter(sobject, record, search));
        }
    ], cb);
  };
}
  ...
  async.waterfall([
      ...
      function retrieveMissingDocs(results, cb) {
        async.map(results, function(result, cb) {
          if(result.doc === null) {
            createSobjectToDocument(config.sfdcOrg, req.sfdcOAuth, search)(result.sobject, function(err, doc) {
